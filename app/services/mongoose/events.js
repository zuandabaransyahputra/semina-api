...
const getAllEvents = async (req) => {
    ...
    if (status) {
        if (status.toLowerCase() === "draft" || status.toLowerCase() === "published") {
            condition = { ...condition, statusEvent: { $regex: status, $options: 'i' } }
        } else {
            throw new NotFoundError('Query status hannya memiliki parameter/keyword Draft atau Published')
        }
    }
